...
process_valstr(ValStr) ->
    {_, TupList] = regexp:split(ValStr, ":"),
    FinalList = lists:map(fun(StrTup) ->
                            {_, OneTuple} = regexp:split(StrTuple, ","),
                            [A,B|_] = OneTuple, {A,B} end, TupList),
    FinalList.

get_biggest_edge(ValidEdges) ->
    lists:foldl(fun({NewC, NewW}, {OldC, OldW}) ->
...
find_new_child(EList, ChildList, VertexValStr) ->
 ...
 %%don't look at edges I have already attempted to claim
            ...
            Result = 
                    case list_to_integer(NewChild) =:= -1 of
                        false ->
                            {VertexValStr ++ ":" ++ NewChild ++ "," ++ NewChildWeight,
                            [{list_to_integer(NewChild), NewChildWeight ++ ":"
                                ++ "parent"} | []];
                        _ ->
                            {VertexValStr, []}
                    end,
...
max_paths({VertexName, VertexValStr, EList}, InMsgs) ->
    ...
    case Snap of
        ...
        _ ->
            ...
            FindNewChildBool = lists:any(fun(CurMessage) ->
                                    CurMessage =:= "reject" end, InMsgs),
            {NewVertexVal, OutMsgsChild} = 
            case FindNewChildBool of
                true ->
                     Result = find_new_child(Elist, ChildList, VertexValStr),
                    {NewVertexVal, OutMsgs};
                _ ->
                    {false, []}
            end,
            
            SplitMessages = lists:filter(fun(CurMessage) ->
                                {_, StrTuple} = regexp:split(CurMessage, ":"),
            ...
            RejectedParents = lists:filter(fun(CurMessage) ->
                                        {CurId, CurWeight} = CurPos,
                                        {MaxId, MaxWeight} = NewParent,
                                        CurId =/= MaxId end, PossibleParents),
            RejectMessages = [{ID, "reject"} || {ID, _} <- RejectedParents],
            OutMsgs = [OutMsgsChild | RejectedParents],
            FinalVertexValue =
            case NewVertexVal of
                ...
                _ ->    [_|TempValStr] = process_valstr(NewVertexVal),
                        ...
                        FinalTuplesStr = string:join(
                            lists:map(fun({ID,Weight}) -> ID ++ "," ++ "Weight" end, FinalValStr),
                            ":"), FinalTupleStr end,
            NewVInfo = {VertexName, FinalVertexVal, Elis},
...
% {_, SplitMessage} = regexp:split(CurMessage, ":"),
